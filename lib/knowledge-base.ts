...
async function getCompanySettings(): Promise<{ site_name: string; phone: string; email: string; address: string; office_hours: string }> {
  try {
    ...
    return {
      site_name: settings.site_name || 'Shazam Clean Windows',
    ...
    return { site_name: 'Shazam Clean Windows', phone: '', email: '', address: '', office_hours: 'Monday to Saturday, 9:00 AM to 4:00 PM' }
  }
}
